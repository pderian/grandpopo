...
def video_info(videofile):
    ...
    return {'file': videofile,
            ...
            'creation_time': datetime.datetime.strptime(
                infos['format']['tags']['creation_time'][:19], '%Y-%m-%dT%H:%M:%S'),
...
def read_video_preprocess_save_img(inputfile, outputdir, preprocessor, prefix='', verbose=True, workers=None):
    """                                                                                                                                                                                                  
    ...
    jsondata = {# metadata
                ...
                'createdBy': __file__,
                'description': 'Data for the frames extracted from video file and pre-processed. Resolution is in [m/px]; xBounds and yBounds in [m] UTM.',
                # source data
    ...
    for i, img in enumerate(processed_frames):
        outputfile = os.path.join(outputdir, '{}{:03d}.png'.format(prefix, i))
        pyplot.imsave(outputfile, img)
...
def read_video_preprocess_save_netcdf(inputfile, outputfile, preprocessor, verbose=True, workers=None):
    ...
    if verbose:
        ...
        sys.stdout.flush
    # allocate output
    processed_frames = numpy.empty((len(data['frames']), preprocessor.y.size, preprocessor.x.size))
    # create pool
    os.system("taskset -p 0xff %d" % os.getpid()) #see http://stackoverflow.com/questions/15639779/why-does-multiprocessing-use-only-a-single-core-after-i-import-numpy
    pool = multiprocessing.Pool(workers, maxtasksperchild=10)
    # create iterator
...
def main(argv):
    ...
    parser.add_argument("-o", "--output", default=None, dest="outputfile", help="netcdf output file / imag eoutput directory (with -i, --img)")
    ...
    parser.add_argument("-i", "--img", dest="asImg", action="store_true", help="export images instead of netCDF file")
    ...
    parser.set_defaults(asImg=False)
    ...
    xmin, xmax = x_UTM - dim/2., x_UTM + dim/2.
    #xmin, xmax = x_UTM - 20., x_UTM + 100.
